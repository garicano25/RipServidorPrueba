...
function mostrar_catalogo(num_catalogo)
{
    ...
    $.each( $('#tabla_lista_catalogos tr'), function( key, value )
    {
   	...
   	switch (catalogo){
        ...
        case 1:
            // activar menu
            $("#titulo_tabla").html('Catálogo [Estructura organizacional]');
            $("#tr_1").addClass("active");
			$("#cat_1").addClass("text-info");

            // Inicializar tabla
            if(datatable_organizacion != null)
            {
                datatable_organizacion.destroy();
                datatable_organizacion = null;
            }
...
$("#boton_nuevo_registro").click(function()
{
    switch (catalogo)
    {
        case 1:
            // Borrar formulario
            $('#form_etiqueta').each(function(){
                this.reset();
            });

            // campos hidden
            $("#ID_ETIQUETA").val(0);
            $("#catalogo_etiqueta").val(catalogo);

            // abrir modal
            $('#modal_etiqueta').modal({backdrop:false});
            break;
            case 2:
                // Borrar formulario
                $('#form_opciones').each(function(){
                    this.reset();
                });
    
                // campos hidden
                $("#ID_OPCIONES_ETIQUETAS").val(0);
                $("#catalogo_opciones").val(catalogo);
...
function tabla_organizacion(num_catalogo)
{
    var ruta = "/recsensorialconsultacatalogo/"+ num_catalogo ;
    ...
    try
    {
        if (datatable_organizacion != null)
        {
            datatable_organizacion.clear().draw();
            datatable_organizacion.ajax.url(ruta).load();
        }
        else
        {
            datatable_organizacion = $('#tabla_lista_organizacion').DataTable({
                "ajax": {
                    "url": ruta,
                    "type": "get",
                    "cache": false,
                    ...
                    {
                        // console.log(xhr); console.log(code);
                        tabla_organizacion(num_catalogo);
                    },
                    "data": {}
                },
                "columns": [
...
$("#boton_guardar_catalogo_etiqueta").click(function() {
    var valida = this.form.checkValidity();
    if (valida) {
        $('#form_etiqueta').ajaxForm({
            ...
            success: function(dato) {
                ...
                $('#ID_ETIQUETA').val(dato.etiqueta.ID_ETIQUETA);  
                ...
                $('#boton_guardar_catalogo_etiqueta').html('Guardar <i class="fa fa-save"></i>');
            },
            beforeSend: function() {
                $('#boton_guardar_catalogo_etiqueta').html('Guardando <i class="fa fa-spin fa-spinner"></i>');
            },
            error: function(dato) {
                $('#boton_guardar_catalogo_etiqueta').html('Guardar <i class="fa fa-save"></i>');
                swal({
...
function editar_catalogoetiqueta()
{
    $('#tabla_lista_organizacion tbody').on('click', 'td.editar', function () {
        
        $('#nav-etiqueta-tab').click()
        // console.log();
        var tr = $(this).closest('tr');
        var row = datatable_organizacion.row(tr);
        ...
        $("#ID_ETIQUETA").val(row.data().ID_ETIQUETA);
        $("#TIPO_ETIQUETA").val(row.data().TIPO_ETIQUETA);
        $("#NOMBRE_ETIQUETA").val(row.data().NOMBRE_ETIQUETA);
        $("#DESCRIPCION_ETIQUETA").val(row.data().DESCRIPCION_ETIQUETA);
...
$("#boton_guardar_etiqueta_opciones").click(function() {
    var valida = this.form.checkValidity();
    if (valida) {
        $('#form_opciones').ajaxForm({
            ...
            type: 'POST',
            url: '/recsensorialcatalogos',
            data: { ETIQUETA_ID: $('#ID_ETIQUETA').val() },  // Incluye el ETIQUETA_ID actualizado
            resetForm: false,
            success: function(dato) {
                swal({
                    title: "Correcto",
...
function tabla_opciones(ID) {
    var ruta = "/opcionesOrganizacion/" + ID;

    if (datatable_opciones) {
        datatable_opciones.ajax.url(ruta).load();

    } else {

      datatable_opciones = $('#tabla_opciones').DataTable({
                "ajax": {
                    "url": ruta,
                    "type": "get",
                    "cache": false,
...
function editar_catalogoetiqueta_opciones()
{
    $('#tabla_lista_opciones tbody').on('click', 'td.editar', function() {

        var tr = $(this).closest('tr');
        var row = datatable_opciones.row(tr);
        ...
        $("#ETIQUETA_ID").val(row.data().ETIQUETA_ID);
        $("#NOMBRE_OPCIONES").val(row.data().NOMBRE_OPCIONES);
