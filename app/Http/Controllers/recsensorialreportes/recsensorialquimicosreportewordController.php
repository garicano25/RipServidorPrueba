...
class recsensorialquimicosreportewordController extends Controller
{
    ...
    public function recsensorialquimicosreporte1word($recsensorial_id)
    {
        ...
        if (count($titulo_partida) > 0) {
            ...
            $plantillaword->setValue('proyecto_portada', str_replace("\n", "<w:br/>", $titulo_partida[0]->clientepartidas_descripción) . ' Con el número de contrato ' . $recsensorial[0]->cliente_numerocontrato . ' (' .  str_replace("\n", "<w:br/>", $contrato->DESCRIPCION_CONTRATO) . ').');
        } else {
        ...
        $sql = DB::select('SELECT hoja.catsustancia_nombre,
                                    ...
                                    IFNULL(
                                        IF(ROUND(relacion.PORCENTAJE, 2) = ROUND(relacion.PORCENTAJE), 
                                    ...
                                    IF(relacion.OPERADOR = "*","",relacion.OPERADOR) as OPERADOR
                        ...
                        GROUP BY relacion.HOJA_SEGURIDAD_ID, 
                                        ...
                                        catvolatilidad_tipo,
                                        OPERADOR
                        ORDER BY hoja.id, sus.SUSTANCIA_QUIMICA', [$recsensorial_id]);
        ...
        $sql = DB::select("SELECT DISTINCT SUMAS.*,
                                (CASE
                                    WHEN SUMAS.SUMA_PONDERACIONES = 3 OR SUMAS.SUMA_PONDERACIONES = 4 THEN 'Muy Baja'
                                ...
                                END) AS PRIORIDAD,
                                (CASE
                                    WHEN SUMAS.SUMA_PONDERACIONES = 3 OR SUMAS.SUMA_PONDERACIONES = 4 THEN '#FDFEFE'
                            ...
                            FROM (
                                SELECT PONDERACIONES.*,
                                    (PONDERACION_CANTIDAD + PONDERACION_CLASIFICACION + PONDERACION_VOLATILIDAD) AS SUMA_PONDERACIONES
                                FROM (
                                    SELECT 
                                        IFNULL(area.recsensorialarea_nombre, 'Sin dato') AS AREA,
                                        hoja.catsustancia_nombre AS PRODUCTO,
                                        sus.SUSTANCIA_QUIMICA AS COMPONENTE,
                                        IF((JSON_CONTAINS(entidad.CONNOTACION, '\"1\"') OR JSON_CONTAINS(entidad.CONNOTACION, '\"2\"')), 5 , f_ponderacion_cantidad(?, inventario.recsensorialcategoria_id, inventario.recsensorialarea_id, sus.ID_SUSTANCIA_QUIMICA)) AS PONDERACION_CANTIDAD,
                                    ...
                                    LEFT JOIN sustanciaQuimicaEntidad entidad ON entidad.SUSTANCIA_QUIMICA_ID = sus.ID_SUSTANCIA_QUIMICA
                                    LEFT JOIN catEntidades catEntidad ON catEntidad.ID_ENTIDAD = entidad.ENTIDAD_ID
                                    LEFT JOIN recsensorialarea area ON inventario.recsensorialarea_id = area.id
                                    LEFT JOIN catvolatilidad vol ON vol.id = relacion.VOLATILIDAD
                                    WHERE inventario.recsensorial_id = ?
                                        AND entidad.ENTIDAD_ID = 1 
                                        AND (relacion.PORCENTAJE > 1.00 OR JSON_CONTAINS(entidad.CONNOTACION, '\"1\"') OR JSON_CONTAINS(entidad.CONNOTACION, '\"2\"'))
                                    ORDER BY area.id ASC, hoja.id ASC
                                ) AS PONDERACIONES
                            ) AS SUMAS", [$recsensorial_id, $recsensorial_id]);
        ...
        $sql = DB::select('SELECT DISTINCT PONDERACION1.*,
                            (CASE
                                    WHEN PONDERACION1.SUMA_PONDERACIONES2 >= 13 THEN "Muy alta"
                                    WHEN PONDERACION1.SUMA_PONDERACIONES2 BETWEEN 9 AND 12 THEN "Alta"
                                    WHEN PONDERACION1.SUMA_PONDERACIONES2 BETWEEN 4 AND 8 THEN "Moderada"
                                    WHEN PONDERACION1.SUMA_PONDERACIONES2 <= 3 THEN "Baja"
                                    ELSE "ND"
                            END) AS PRIORIDAD,
                            (CASE
                                    WHEN PONDERACION1.SUMA_PONDERACIONES2 >= 13 THEN "#8E44AD" -- MORADO
                                    WHEN PONDERACION1.SUMA_PONDERACIONES2 BETWEEN 9 AND 12 THEN "#E74C3C" -- ROJO
                                    WHEN PONDERACION1.SUMA_PONDERACIONES2 BETWEEN 4 AND 8 THEN "#F1C40F" -- AMARILLO
                                    WHEN PONDERACION1.SUMA_PONDERACIONES2 <= 3 THEN "#2ECC71" -- VERDE
                                    ELSE "#000000" -- NEGRO
                            END) AS COLOR
                    FROM
                    (SELECT SUMAS.*,
                            (PONDERACION_CANTIDAD + PONDERACION_CLASIFICACION + PONDERACION_VOLATILIDAD) AS SUMA_PONDERACIONES1,
                            (PONDERACION_VIAINGRESO + PONDERACION_POE + PONDERACION_EXPOSICION) AS SUMA_PONDERACIONES2
                    FROM 
                    ( SELECT 
                                    IFNULL(area.recsensorialarea_nombre, "Sin dato") AS AREA,
                                    ...
                                    hoja.catsustancia_nombre as PRODUCTO,
                                    sus.SUSTANCIA_QUIMICA AS COMPONENTE,
                                    "PRIMER PONDERACION " AS PRIMER_PONDERACION,
                                    IF((JSON_CONTAINS(entidad.CONNOTACION, \'\"1\"\') OR JSON_CONTAINS(entidad.CONNOTACION, \'\"2\"\')), 5 , f_ponderacion_cantidad(?, inventario.recsensorialcategoria_id, inventario.recsensorialarea_id, sus.ID_SUSTANCIA_QUIMICA) ) AS PONDERACION_CANTIDAD,
                                    ((inventario.recsensorialquimicosinventario_cantidad * relacion.PORCENTAJE)/100) AS PORCENTAJE,						
                                    (CASE
                                            WHEN sus.CLASIFICACION_RIESGO = 0 THEN 1
                                    ...
                                    END) AS PONDERACION_CLASIFICACION,
                                    vol.catvolatilidad_ponderacion as PONDERACION_VOLATILIDAD,
                                    "SEGUNDA PONDERACION" AS SEGUNDA_PONDERACION,
                                    ingreso.catviaingresoorganismo_ponderacion AS PONDERACION_VIAINGRESO,
                                    (CASE
                                                WHEN areaCategoria.recsensorialareacategorias_total > 100 THEN 8
                                                WHEN areaCategoria.recsensorialareacategorias_total BETWEEN 25 AND 100 THEN 4
                                                WHEN areaCategoria.recsensorialareacategorias_total BETWEEN 5 AND 24 THEN 2
                                                WHEN areaCategoria.recsensorialareacategorias_total < 5 THEN 1
                                                ELSE 0
                                        END) AS PONDERACION_POE,
                                        (CASE
                                                WHEN ((IFNULL(inventario.recsensorialcategoria_tiempoexpo, 0) * IFNULL(inventario.recsensorialcategoria_frecuenciaexpo, 0))/60) >= 7 THEN 8
                                                WHEN ((IFNULL(inventario.recsensorialcategoria_tiempoexpo, 0) * IFNULL(inventario.recsensorialcategoria_frecuenciaexpo, 0))/60) >= 3 THEN 4
                                                WHEN ((IFNULL(inventario.recsensorialcategoria_tiempoexpo, 0) * IFNULL(inventario.recsensorialcategoria_frecuenciaexpo, 0))/60) >= 1 THEN 2
                                                ELSE 1
                                        END) AS PONDERACION_EXPOSICION
                            FROM recsensorialquimicosinventario inventario
                            LEFT JOIN catHojasSeguridad_SustanciasQuimicas relacion ON relacion.HOJA_SEGURIDAD_ID = inventario.catsustancia_id
                            LEFT JOIN catsustancia hoja ON hoja.id = relacion.HOJA_SEGURIDAD_ID
                            LEFT JOIN catsustancias_quimicas sus ON sus.ID_SUSTANCIA_QUIMICA = relacion.SUSTANCIA_QUIMICA_ID
                            ...
                            WHERE inventario.recsensorial_id = ?  
                                    AND entidad.ENTIDAD_ID = 1 
                                    AND (relacion.PORCENTAJE > 1.00 OR 
                                                JSON_CONTAINS(entidad.CONNOTACION, \'\"1\"\') OR 
                                                JSON_CONTAINS(entidad.CONNOTACION, \'\"2\"\'))
                            ORDER BY area.id ASC, hoja.id ASC
                    ) AS SUMAS
                    ) AS PONDERACION1
                    WHERE PONDERACION1.SUMA_PONDERACIONES1 >= 8 ', [$recsensorial_id, $recsensorial_id]);
        ...
        $sql = DB::select('SELECT DISTINCT PONDERACION2.*,
            (CASE  
                WHEN PONDERACION2.SUMA_PONDERACIONES2 >= 13 THEN
                        CASE
                            WHEN NUM_TRABAJADORES > 100 THEN 20
                            WHEN NUM_TRABAJADORES >= 51 THEN 15
                            WHEN NUM_TRABAJADORES >= 26 THEN 8
                            WHEN NUM_TRABAJADORES >= 16 THEN 5
                            WHEN NUM_TRABAJADORES >= 9 THEN 3
                            WHEN NUM_TRABAJADORES >= 3 THEN 2
                            ELSE 1
                    END
                ELSE
                    CASE
                            WHEN NUM_TRABAJADORES > 100 THEN 10
                            WHEN NUM_TRABAJADORES >= 51 THEN 7
                            WHEN NUM_TRABAJADORES >= 31 THEN 5
                            WHEN NUM_TRABAJADORES >= 21 THEN 4
                            WHEN NUM_TRABAJADORES >= 11 THEN 3
                            WHEN NUM_TRABAJADORES >= 6 THEN 2
                            ELSE 1
                    END
                END) AS NUM_POE
            FROM 
            ...
            ( SELECT 
                ...
                (CASE
                        ...
                        ELSE 1
                END) AS PONDERACION_EXPOSICION,
                "TERCERA PONDERACION" AS TERCERA_PONDERACION,
                areaCategoria.recsensorialareacategorias_total AS NUM_TRABAJADORES,
                IFNULL(inventario.recsensorialcategoria_tiempoexpo, 0) AS TIEMPO_EXPO,
                IFNULL(inventario.recsensorialcategoria_frecuenciaexpo, 0) AS FRECUENCIA_EXPO,
                ...
                categoria.sumaHorasJornada AS horas_jornada
            FROM recsensorialquimicosinventario inventario
            LEFT JOIN catHojasSeguridad_SustanciasQuimicas relacion ON relacion.HOJA_SEGURIDAD_ID = inventario.catsustancia_id
            LEFT JOIN catsustancia hoja ON hoja.id = relacion.HOJA_SEGURIDAD_ID
            LEFT JOIN catsustancias_quimicas sus ON sus.ID_SUSTANCIA_QUIMICA = relacion.SUSTANCIA_QUIMICA_ID
            ...
            WHERE PONDERACION1.SUMA_PONDERACIONES1 >= 8 AND PONDERACION1.SUMA_PONDERACIONES2 >= 4) AS PONDERACION2 ', [$recsensorial_id, $recsensorial_id]);
        ...
        $sql = DB::select('SELECT
                                ...
                                IFNULL(tabla.PPT_NUEVO, 0) MUESTREO_PPT,
                                IFNULL(tabla.CT_NUEVO, 0) MUESTREO_CT,
                                tabla.PUNTOS_NUEVO TOTAL_MUESTREOS
                            ...
                            GROUP BY tabla.ID_TABLA_INFORME,
                                    ...
                                    MUESTREO_CT,
                                    TOTAL_MUESTREOS
                            ORDER BY tabla.ID_TABLA_INFORME, CATEGORIA_ID', [$recsensorial_id]);
        ...
        // TABLA 12.1 - RESUMEN VERSION 2 [PUNTOS DE MUESTREOS (CLIENTE)] -- NO SE OCUPA
        //================================================================================
        ...
        // TABLA 12.1 Controles para agentes químicos con los que cuenta el área - NO SE OCUPA
        //================================================================================
        ...
        // TABLA 14.2 - RESUMEN VERSION 2 [PUNTOS DE MUESTREOS Y TIPO DE INSTALACION (CLIENTE)] - NO SE OCUPA
        //================================================================================
