...
<body>
            ...
            <tbody>
                <tr>
                    ...
                    <td rowspan="2" width="60%">
                        <h1>Lista de signatarios</h1>

                        @if(isset($signatarioslista['proyectosignatario_revision']) && $signatarioslista['proyectosignatario_revision'] > 0)
                        (Rev-{{ $signatarioslista['proyectosignatario_revision'] }})
                        @endif

                        @if(isset($signatarioslista['proyectosignatario_cancelado']) && $signatarioslista['proyectosignatario_cancelado'] > 0)
                        <h1 style="color: #F00;">(CANCELADO)</h1>
                        @endif
                    </td>
                    <td width="20%">
                        @if(isset($signatarioslista['created_at']))
                            <b>Fecha:</b><br>{{ date('Y-m-d', strtotime($signatarioslista['created_at'])) }}
                        @endif
                    </td>
                </tr>
                <tr>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" id="tabla_listaservicios">
        <thead>
            <tr>
                <th colspan="4">Signatarios asignados</th>
            </tr>
            <tr>
                <th style="width: 5%!important;">No.</th>
                <th>Proveedor</th>
                <th style="width: 35%!important;">Nombre</th>
                <th style="width: 35%!important;">Puesto</th>
            </tr>
        </thead>
        <tbody>
            @php $fila = 0; @endphp
            ...
            <tr>
                <td>{{ $fila }}</td>
                <td>{{ $signatario->proveedor_NombreComercial }}</td>
                <td>
                    {{ $signatario->signatario_Nombre }}<br>
                    <span style="font-size: 10px; color: #888888;"><b>RFC: </b>{{ $signatario->signatario_Rfc }}</span>

                    @if($proyecto->catregion_id == 3) {{-- REGION MARINA --}}
                    <br><span style="font-size: 10px; color: #888888;"><b>Lib. de Mar: </b>{{ $signatario->libreta_mar }}</span>
                    @endif
                </td>
                <td>{{ $signatario->signatario_Cargo }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div id="firma">
        @if($signatarioslista['proyectosignatario_autorizado'] > 0)
        <div style="border: 1px #555555 solid; width: 220px; margin: 0px auto 4px auto">
            Documento firmado digitalmente por:<br>
            {{ $signatarioslista['proyectosignatario_autorizadonombre'] }}<br>
            {{ $signatarioslista['proyectosignatario_autorizadofecha'] }}
        </div>
        @else
        <br>
        @endif
