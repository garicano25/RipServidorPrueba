...
<div class="tab-pane active" role="tabpanel" id="tab_parametro_1">
    @if(auth()->user()->hasRoles(['Superusuario', 'Administrador', 'Coordinador','Operativo HI', 'Ergónomo']) && ($recsensorial->recsensorial_bloqueado + 0) == 0 && ($recsensorial->recsensorial_fisicosimprimirbloqueado + 0) == 0)
    ...
    <table class="table table-hover stylish-table" width="100%" id="tabla_parametro">
        <thead>
            <tr>
                ...
                <th>Categoría</th>
                <th>Área (s)</th>
                <th style="width: 100px!important;">Movimiento<br>repetitivo</th>
                <th style="width: 100px!important;">Postura<br>mantenida</th>
                <th style="width: 100px!important;">Postura<br>forzada</th>
                <th style="width: 100px!important;">Carga<br>manual</th>
                <th style="width: 100px!important;">Fuerza</th>
                {{-- <th>Evaluar</th> --}}
        ...
        <div class="modal-content">
            <form enctype="multipart/form-data" method="post" name="form_parametro" id="form_parametro">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="modal_titulo">Riesgos ergonómicos</h4>
                </div>
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-2">
                            <div class="form-group">
                                ...
                                <select class="custom-select form-control" id="parametroergonomia_movimientorepetitivo" name="parametroergonomia_movimientorepetitivo" required>
                                    <option value=""></option>
                        ...
                        <div class="col-2">
                            <div class="form-group">
                                <label>Post. Mantenida *</label>
                                <select class="custom-select form-control" id="parametroergonomia_posturamantenida" name="parametroergonomia_posturamantenida" required>
                                    <option value=""></option>
                                    <option value="1">Si</option>
                                    <option value="0">No</option>
                                </select>
                        ...
                        <div class="col-2">
                            <div class="form-group">
                                <label>Post. forzada *</label>
                                <select class="custom-select form-control" id="parametroergonomia_posturaforzada" name="parametroergonomia_posturaforzada" required>
                                    <option value=""></option>
                                    <option value="1">Si</option>
                                    <option value="0">No</option>
                                </select>
                        ...
                        <div class="col-2">
                            <div class="form-group">
                                <label>Fuerza *</label>
                                <select class="custom-select form-control" id="parametroergonomia_fuerza" name="parametroergonomia_fuerza" required>
                                    <option value=""></option>
                                    <option value="1">Si</option>
                                    <option value="0">No</option>
                                </select>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <h4 class="card-title">Áreas que recorre</h4>
                            <hr>
                        </div>
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    <div class="row" id="inputs_form_area">
                ...
                <div class="modal-footer">
                    ...
                    @if(auth()->user()->hasRoles(['Superusuario', 'Administrador', 'Coordinador','Operativo HI','Ergónomo']) && ($recsensorial->recsensorial_bloqueado + 0) == 0)
...
<script type="text/javascript" charset="utf-8" async defer>
    ...
    function funcion_tabla_parametro(recsensorial_id) {
        tabla_parametro = $('#tabla_parametro').DataTable({
            "ajax": {
                "url": "/parametroergonomiatabla/" + recsensorial_id,
                "type": "get",
                "cache": false,
                error: function(xhr, error, code) {
                    // console.log(xhr); console.log(code);
                    // funcion_tabla_parametro(recsensorial_id);
                    tabla_parametro.ajax.url("/parametroergonomiatabla/" + recsensorial_id).load();
                },
                "data": {}
            },
            "columns": [
                ...
                {
                    "data": "movimientorepetitivo"
                },
                {
                    "data": "posturamantenida"
                },
                {
                    "data": "posturaforzada"
                },
    ...
    $("#boton_nuevo_parametro").click(function() {
        ...
        consulta_ergonomiaareas(recsensorial_id, 0);
    ...
    // consulta areas parametro ergonomia
    function consulta_ergonomiaareas(recsensorial_id, parametroergonomia_id) {
        // alert(recsensorial_id+' - '+parametroergonomia_id);
        $.ajax({
            type: "GET",
            dataType: "json",
            url: "/ergonomiaareas/" + recsensorial_id + "/" + parametroergonomia_id,
            data: {},
            cache: false,
            success: function(dato) {
                $('#inputs_form_area').html('');
                $.each(dato.ergonomiaareas, function(key, value) {
                    $("#inputs_form_area").append('<div class="col-6">' +
                        '<div class="form-group">' +
                        '<div class="switch" style="float: left;">' +
                        '<label>' +
                        '<input type="checkbox" name="ergonomiaarea[]" value="' + value.id + '" ' + value.seleccionado + '>' +
                        '<span class="lever switch-col-light-blue"></span>' +
                        '</label>' +
                        '</div>' +
                        '<label class="demo-switch-title" style="float: left;">' + value.recsensorialarea_nombre + '</label>' +
                        '</div>' +
                        '</div>');
                });
            },
            beforeSend: function() {
                $('#inputs_form_area').html('<tr><td colspan="10" align="center"><i class="fa fa-spin fa-spinner fa-5x"></i></td></tr>');
            },
            error: function(dato) {
                $('#inputs_form_area').html('Error al consultar áreas, intentelo de nuevo.');
                return false;
            }
        }); //Fin ajax
    }
    ...
    $(document).ready(function() {
        $('#tabla_parametro tbody').on('click', 'td.editar', function() {
            ...
            if (parseInt(row.data().accion_activa) > 0) {
                ...
                $("#parametroergonomia_movimientorepetitivo").val(row.data().parametroergonomia_movimientorepetitivo);
                $("#parametroergonomia_posturamantenida").val(row.data().parametroergonomia_posturamantenida);
                $("#parametroergonomia_posturaforzada").val(row.data().parametroergonomia_posturaforzada);
                $("#parametroergonomia_fuerza").val(row.data().parametroergonomia_fuerza);
                $("#parametroergonomia_cargamanual").val(row.data().parametroergonomia_cargamanual);
                consulta_select_categorias(recsensorial_id, row.data().recsensorialcategoria_id);

                // mostrar lista de areas de este registro
                consulta_ergonomiaareas(row.data().recsensorial_id, row.data().id);
    ...
    $("#boton_guardar_parametro").click(function() {
        ...
        if (valida) {
            ...
            if (seleccionados > 0) {
                ...
                // mensaje
                swal({
                    title: "Seleccione Área (s)",
                    text: "Debe Seleccionar al menos un área",
                        ...
                        swal({
                            ...
                            if (isConfirm) {
                                ...
                                $.ajax({
                                    type: "GET",
                                    dataType: "json",
                                    url: "/parametroergonomiaeliminar/" + row.data().id,
                                    data: {},
                                    cache: false,
                                    success: function(dato) {
                                        // actualiza tabla
    ...
    var parametro_id = 13; // ID en base datos
    var parametro_nombre = 'riesgos ergonómicos';
