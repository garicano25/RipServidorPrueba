    ...
    <table class="table table-hover stylish-table" width="100%" id="tabla_parametro">
        <thead>
            <tr>
                ...
                <th style="width: 220px!important;">Análisis</th>
                <th style="width: 100px!important;">Puntos</th>
        ...
        <div class="modal-content">
            <form enctype="multipart/form-data" method="post" name="form_parametro" id="form_parametro">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="modal_titulo">Alimento</h4>
                </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <label> Área *</label>
                                <select class="custom-select form-control" id="select_area" name="recsensorialarea_id" required>
                                    <option value=""></option>
                                </select>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="parametroalimento_ubicacion" id="parametroalimento_ubicacion" required>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
                                <label> Puntos *</label>
                                <input type="number" class="form-control" name="parametroalimento_puntos" id="parametroalimento_puntos" required>
                            </div>
                        </div>
                        <div class="col-12">
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="row" id="chekbox_caracteristicas"></div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal-dialog modal-lg" style="min-width: 900px!important;">
        <form method="post" enctype="multipart/form-data" name="form_evidencia_fotos" id="form_evidencia_fotos">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Cerrar</button>
                    @if(auth()->user()->hasRoles(['Superusuario', 'Administrador', 'Coordinador'])  && ($recsensorial->recsensorial_bloqueado + 0) == 0)
...
<script type="text/javascript" charset="utf-8" async defer>
    ...
    function consulta_opciones_caracteristicas(seleccionado_id)
    {
        $.ajax({
            type: "GET",
            dataType: "json",
            url: "/parametroalimentocaracteristicas/"+seleccionado_id,
            data:{},
            cache: false,
            success:function(dato){
                ...
                {
                    if (parseInt(seleccionado_id) == 0)
                    {
                        value.checked = "checked";
                    }

                    // Dibujar checkboxs
                    $("#chekbox_caracteristicas").append('<div class="col-6">'+
                                                            '<div class="form-group">'+
                                                                '<div class="switch" style="float: left;">'+
                                                                    '<label>'+
                                                                        '<input type="checkbox" name="caracteristica[]" id="chekbox_caracteristica_'+value.id+'" value="'+value.id+'" '+value.checked+'>'+
                                                                        '<span class="lever switch-col-light-blue"></span>'+
                                                                    '</label>'+
                                                                '</div>'+
                                                                '<label class="demo-switch-title" style="float: left;">'+value.catparametroalimentocaracteristica_caracteristica+'</label>'+
                                                            '</div>'+
                                                        '</div>');
                });
    ...
    function funcion_tabla_parametro(recsensorial_id)
    {
        tabla_parametro = $('#tabla_parametro').DataTable({
            "ajax": {
                "url": "/parametroalimentotabla/"+recsensorial_id,
                "type": "get",
                "cache": false,
                error: function (xhr, error, code)
                {
                    ...
                    // funcion_tabla_parametro(recsensorial_id);
                    tabla_parametro.ajax.url("/parametroalimentotabla/"+recsensorial_id).load();
                },
                "data": {}
            },
            "columns": [
                ...
                {
                    "data": "parametroalimento_ubicacion",
                    "defaultContent": "-"
                },
                // {
                //     "data": "catparametroalimentocaracteristica_tipo",
                //     "defaultContent": "-"
                // },
                {
                    "data": "lista_caracteristicas_texto",
                    "defaultContent": "-"
                },
                {
                    "data": "parametroalimento_puntos",
                    "defaultContent": "-"
                },
                {
                    "className": 'editar',
    ...
    $("#boton_nuevo_parametro").click(function()
    {
        ...
        consulta_opciones_caracteristicas(0);
    ...
    $(document).ready(function()
    {
        $('#tabla_parametro tbody').on('click', 'td.editar', function() 
        {
            ...
            if (parseInt(row.data().accion_activa) > 0)
            {
                ...
                $("#parametroalimento_ubicacion").val(row.data().parametroalimento_ubicacion);
                // $("#parametroalimento_caracteristica").val(row.data().parametroalimento_caracteristica);
                $("#parametroalimento_puntos").val(row.data().parametroalimento_puntos);
                consulta_select_areas(recsensorial_id, row.data().recsensorialarea_id);
                consulta_opciones_caracteristicas(row.data().id);
        ...
        if (valida)
        {
            ...
            else
            {
                ...
                swal({
                    ...
                    text: "Debe Seleccionar al menos una análisis",
                        ...
                        swal({   
                            ...
                            if (isConfirm)
                            {
                                ...
                                $.ajax({
                                    type: "GET",
                                    dataType: "json",
                                    url: "/parametroalimentoeliminar/"+row.data().id,
                                    data:{},
                                    cache: false,
                                    success:function(dato){
                                        // actualiza tabla
    ...
    var parametro_id = 11; // ID en base datos
    var parametro_nombre = 'alimentos';
    ...
    function descripcion_foto()
    {
        if(parseInt($("#recsensorialevidencias_tipo").val()) > 0)
        {
            if(parseInt($("#recsensorialevidencias_tipo").val()) == 1)
            {
                ...
                {
                    $("#recsensorialevidencias_descripcion").val('Evidencia de reconocimiento de la calidad de los '+parametro_nombre+' en '+$("#recsensorialevidencias_recsensorialarea_id option:selected").text());
                }
                else
                {
                    $("#recsensorialevidencias_descripcion").val('Evidencia de reconocimiento de la calidad de los '+parametro_nombre);
                }
            }
            else
            {
