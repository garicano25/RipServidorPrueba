    ...
    <table class="table table-hover stylish-table" width="100%" id="tabla_parametro">
        <thead>
            <tr>
                ...
                <th>Área</th>
                <th>Ubicación</th>
                <th>Características a medir</th>
                <th style="width: 70px!important;">Puntos</th>
        ...
        <div class="modal-content">
            <form enctype="multipart/form-data" method="post" name="form_parametro" id="form_parametro">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="modal_titulo">Ventilación y calidad del aire</h4>
                </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="parametrocalidadaire_ubicacion" id="parametrocalidadaire_ubicacion" required>
                            </div>
                        </div>
                        <div class="col-12">
                            <label>Características a medir *</label>
                            <div class="card">
                                <div class="card-body">
                                    <div class="row" style="border: 0px #000 solid; max-height: 170px; overflow-x: hidden; overflow-y: auto;">
                                        @foreach($airecaracteristicas as $caracteristica)
                                            <div class="col-6">
                                                <div class="switch" style="float: left;">
                                                    <label>
                                                        <input type="checkbox" class="checkbox_airecaracteristica" name="airecaracteristica[]" id="checkbox_airecaracteristica_{{$caracteristica->id}}" value="{{$caracteristica->id}}">
                                                        <span class="lever switch-col-light-blue"></span>
                                                    </label>
                                                </div>
                                                <label class="demo-switch-title" style="float: left;">{{$caracteristica->catparametrocalidadairecaracteristica_caracteristica}}</label>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <label> Puntos *</label>
                                <input type="number" class="form-control" name="parametrocalidadaire_puntos" id="parametrocalidadaire_puntos" required>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript" charset="utf-8" async defer>
    ...
    function funcion_tabla_parametro(recsensorial_id)
    {
        tabla_parametro = $('#tabla_parametro').DataTable({
            "ajax": {
                "url": "/parametrocalidadairetabla/"+recsensorial_id,
                "type": "get",
                "cache": false,
                error: function (xhr, error, code)
                {
                    ...
                    // funcion_tabla_parametro(recsensorial_id);
                    tabla_parametro.ajax.url("/parametrocalidadairetabla/"+recsensorial_id).load();
                },
                "data": {}
            },
            "columns": [
                ...
                {
                    "data": "parametrocalidadaire_ubicacion",
                    "defaultContent": '-'
                },
                {
                    "data": "airecaracteristicas",
                    "defaultContent": '-'
                },
                {
                    "data": "parametrocalidadaire_puntos",
                    "defaultContent": '-'
                },
                {
                    "className": 'editar',
    ...
    $("#boton_nuevo_parametro").click(function()
    {
        ...
        // Seleccionar opciones analisis checkboxs
        $('.checkbox_airecaracteristica').each(function()
        {
    ...
    $(document).ready(function()
    {
        $('#tabla_parametro tbody').on('click', 'td.editar', function() 
        {
            ...
            if (parseInt(row.data().accion_activa) > 0)
            {
                ...
                $("#parametrocalidadaire_ubicacion").val(row.data().parametrocalidadaire_ubicacion);
                $("#parametrocalidadaire_puntos").val(row.data().parametrocalidadaire_puntos);

                // Activar caracteristicas elegidas
                $.each( row.data().caracteristicas, function( key, value )
                {
                    $("#checkbox_airecaracteristica_"+value.catparametrocalidadairecaracteristica_id).prop('checked', true);
                });
    ...
    $("#boton_guardar_parametro").click(function()
    {
        ...
        if (valida)
        {
            // Verificar si ha seleccionado checkbox caracteristicas del aire
            var seleccionados = 0;
            $('.checkbox_airecaracteristica').each(function(){
                if (this.checked) {
                    seleccionados += 1;
            ...
            else
            {
                // mensaje
                swal({
                    title: "Seleccione características",
                    text: "Seleccione una o más características a medir en el aire.",
                        ...
                        swal({   
                            ...
                            if (isConfirm)
                            {
                                ...
                                $.ajax({
                                    type: "GET",
                                    dataType: "json",
                                    url: "/parametrocalidadaireeliminar/"+row.data().id,
                                    data:{},
                                    cache: false,
                                    success:function(dato){
                                        // actualiza tabla
    ...
    var parametro_id = 8; // ID en base datos
    var parametro_nombre = 'ventilación y calidad del aire';
