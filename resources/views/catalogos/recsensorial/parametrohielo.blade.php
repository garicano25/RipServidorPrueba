        ...
        <div class="modal-content">
            <form enctype="multipart/form-data" method="post" name="form_parametro" id="form_parametro">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="modal_titulo">Hielo</h4>
                </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="parametrohielo_ubicacion" id="parametrohielo_ubicacion" required>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="form-group">
                                <label> Característica *</label>
                                <select class="custom-select form-control" id="catparametrohielocaracteristica_id" name="catparametrohielocaracteristica_id" onchange="consulta_opciones_caracteristicas(this.value, 0);" required>
                                    {{-- <option value=""></option> --}}
                                </select>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="form-group">
                                <label> Puntos *</label>
                                <input type="number" class="form-control" name="parametrohielo_puntos" id="parametrohielo_puntos" required>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript" charset="utf-8" async defer>
    ...
    function consulta_select_caracteristicas(seleccionado_id) {
        $.ajax({
            type: "GET",
            dataType: "json",
            url: "/parametrohieloselectcaracteristicas/" + seleccionado_id,
            data: {},
            cache: false,
            success: function(dato) {
                $("#catparametrohielocaracteristica_id").html(dato.opciones);
            },
            error: function(dato) {
                // alert('Error: '+dato.msj);
                return false;
    ...
    function consulta_opciones_caracteristicas(caracteristica_id, lista_seleccionados) {
        if (caracteristica_id) {
            $.ajax({
                type: "GET",
                dataType: "json",
                url: "/parametrohielocaracteristicas/" + $("#catparametrohielocaracteristica_id option[value='" + caracteristica_id + "']").text(),
                data: {},
                cache: false,
                success: function(dato) {
                    ...
                    $.each(dato.opciones, function(key, value) {
                        ...
                        $("#chekbox_caracteristicas").append('<div class="col-4">' +
                            ...
                            '</label>' +
                            '</div>' +
                            '<label class="demo-switch-title" style="float: left;">' + value.catparametrohielocaracteristica_caracteristica + '</label>' +
                            '</div>' +
                            '</div>');
                    });
    ...
    function funcion_tabla_parametro(recsensorial_id) {
        tabla_parametro = $('#tabla_parametro').DataTable({
            "ajax": {
                "url": "/parametrohielotabla/" + recsensorial_id,
                "type": "get",
                "cache": false,
                error: function(xhr, error, code) {
                    // console.log(xhr); console.log(code);
                    // funcion_tabla_parametro(recsensorial_id);
                    tabla_parametro.ajax.url("/parametrohielotabla/" + recsensorial_id).load();
                },
                "data": {}
            },
            "columns": [
    ...
    $(document).ready(function() {
        $('#tabla_parametro tbody').on('click', 'td.editar', function() {
            ...
            if (parseInt(row.data().accion_activa) > 0) {
                ...
                $("#parametrohielo_ubicacion").val(row.data().parametrohielo_ubicacion);
                $("#catparametrohielocaracteristica_id").val(row.data().catparametrohielocaracteristica_id);
                $("#parametrohielo_puntos").val(row.data().parametrohielo_puntos);
                consulta_select_areas(recsensorial_id, row.data().recsensorialarea_id);
                consulta_opciones_caracteristicas(row.data().catparametrohielocaracteristica_id, row.data().lista_seleccionados);
                        ...
                        swal({
                            ...
                            if (isConfirm) {
                                ...
                                $.ajax({
                                    type: "GET",
                                    dataType: "json",
                                    url: "/parametrohieloeliminar/" + row.data().id,
                                    data: {},
                                    cache: false,
                                    success: function(dato) {
                                        // actualiza tabla
    ...
    var parametro_id = 10; // ID en base datos
    var parametro_nombre = 'hielo';
